...
- 修正同一個對象沒有同時比對「顯示名稱」與「通知 ID」的問題。
- 通知比對會讀取 sender / title 類欄位，並針對 Instagram 這類格式從文字欄位抽出寄件者片段。
- 例如通知顯示 `寶寶2號 · now` 時，會抽出 `寶寶2號` 參與比對。
- 仍不直接掃描完整訊息內容，降低「訊息裡提到某人」造成誤觸發的機率。
...
SimpAlarm 會監聽已選擇 App 的通知，並使用通知中的 sender / title 類欄位來比對對象名稱。
...
App 不會直接比對完整訊息內容，避免別人在訊息中提到某個名字時誤觸發。

針對部分 App 的通知格式，App 也會從文字欄位中抽出像寄件者的片段。例如：
...
這些名稱會視為同一個對象。只要通知 sender / title 類欄位或抽出的寄件者片段出現其中一個，就會觸發同一個對象。
